...
function isWheelchairType(tripType) {
  return tripType === "HASWC" || tripType === "NEEDWC" || tripType === "WC";
}

function qualifiesForGMTD(groupedTrip, tripType, ctx = {}) {
  const derived = ctx.derived || {};
  const inLoop = !!derived.inLoop;
...
function gmtdRule(groupedTrip, rateRow, tripType, ctx = {}) {
  ...
  const roundTripMilesRaw = Number(
    ctx.derived?.gmtdRoundTripMilesRaw ?? groupedTrip.DirectMileage ?? 0
  );
  ...
  const table = isWheelchairType(tripType) ? WHEELCHAIR : AMBULATORY;
